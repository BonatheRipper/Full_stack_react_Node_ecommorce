...
const prices = [
  ...
  { name: "$51 to $2000", value: "51-200" },
  { name: "$201 to $1000", value: "201-200 0" },
];
const ratings = [
...
const SearchScreen = () => {
  ...
  const sp = new URLSearchParams(search); //search category or shirt
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [page, error, price, order, query, rating, category]);
          ...
          <div>
            ...
            <ul>
              {ratings.map((r) => (
                <li key={r.name}>
                  <Link
                    className={`${r.name}` === `${rating}` ? "text-bold" : ""}
